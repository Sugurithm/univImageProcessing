...
Mat changeHandColor(const Mat& currentFrame, const Mat& prevFrame, const Mat& handMask, size_t totalPixels) {
    ...
    // フレーム間差分
    absdiff(grayCurrent, grayPrev, diffFrame);
    threshold(diffFrame, diffFrame, 100, 255, THRESH_BINARY);
    ...
    // 手の領域だけを取り出す
    Mat resultFrame = currentFrame.clone();

    // 動いていれば赤色，動いていなければ元の色
    if (movingArea > totalPixels * 0.01) {
        resultFrame.setTo(Scalar(0, 0, 255), handMask); // 動いている → 赤色
    }
    // 動いていなければ何もせずそのまま（元の画像が残ってる）
    ...
    resize(wallpaperImage, bgResized, currentFrame.size());
...
int main() {
    ...
    while (true) {
        ...
        // 結合
        Mat combinedImage;
        hconcat(vector<Mat>{filteredMask2, divider, filteredMask3, divider, coloredHandFrame}, combinedImage);
