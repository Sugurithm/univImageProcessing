...
Mat visualizeEdgeCount(const Mat& mask) {
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        approxPolyDP(contours[i], approxContours[i], 0.1, true);
    }
...
Mat filterMaskByEdges(const Mat& inputMask) {
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        vector<Point> approx;
        approxPolyDP(contours[i], approx, 0.1, true);
...
int main() {
    ...
    // cvtColor(filteredMask2, filteredMask2, COLOR_GRAY2BGR);
    cvtColor(filteredMask3, filteredMask3, COLOR_GRAY2BGR);
